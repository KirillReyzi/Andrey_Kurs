...
void converter_to_postfix(Stack& stack, Queue& queue, char tmp[], int index)
{
    ...
    while (i < index)
    {
        ...
        if (Prior(data) == 0)
        {
            ...
            if (queue.isEmpty()) { while (tmp[i] != '+' && tmp[i] != '-' && tmp[i] != '/' && tmp[i] != '*' && tmp[i] != '(' && tmp[i] != ')' && i != index) { data += tmp[i]; i++; } queue.add(data); }

            else { while (tmp[i] != '+' && tmp[i] != '-' && tmp[i] != '/' && tmp[i] != '*' && tmp[i] != '(' && tmp[i] != ')' && i != index) { data += tmp[i]; i++; } queue.push_back(data); }
        }
        else
            //2
